...
-- Holds were added after the jobs table.
CREATE TABLE holds (
  id bigint PRIMARY KEY,
...
-- A job with a hold is on hold, whatever jobs.status says.
...
DECLARE
  v_on_hold boolean;
BEGIN
  SELECT EXISTS (SELECT 1 FROM holds h WHERE h.job_id = p_job_id) INTO v_on_hold;
